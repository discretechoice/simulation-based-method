...
design.5.rows <- read.table("data.block2.txt", header=TRUE) 


# Setting parameters
n.individuals         <- 243                  # Number of individuals
...
OBSERV.vector.pattern.11111 <- 17/ n.individuals  # Tested pattern (1,1,1,1,1)
OBSERV.vector.pattern.22222 <- 8/ n.individuals   # Tested pattern (2,2,2,2,2)
OBSERV.vector.pattern.33333 <- 12/ n.individuals  # Tested pattern (3,3,3,3,3)
OBSERV.vector.pattern.44444 <- 20/ n.individuals  # Tested pattern (4,4,4,4,4)
OBSERV.vector.pattern.21123 <- 56/ n.individuals  # Tested pattern (2,1,1,2,3)
OBSERV.vector.pattern.23123 <- 13/ n.individuals  # Tested pattern (2,3,1,2,3)
...
vector.pattern.21123 <- rep(0,n.iteraciones)  # Tested pattern (2,1,1,2,3)
vector.pattern.23123 <- rep(0,n.iteraciones)  # Tested pattern (2,3,1,2,3)
      ...
      data.RPL$choice.Step3   <- apply(cbind(data.RPL$utility1.Step3, data.RPL$utility2.Step3, data.RPL$utility3.Step3, data.RPL$utility4.Step3),1,which.max)    # Generating choice
	    ...
	    for (ind in (1:n.individuals)){
      	...
      	if(all(data.RPL$choice.Step3[rows.individuo] == c(1,1,1,1,1))){data.RPL$patternStep3[rows.individuo] <- 11111}
	    	...
	    	if(all(data.RPL$choice.Step3[rows.individuo] == c(2,1,1,2,3))){data.RPL$patternStep3[rows.individuo] <- 21123}
	    	if(all(data.RPL$choice.Step3[rows.individuo] == c(2,3,1,2,3))){data.RPL$patternStep3[rows.individuo] <- 23123}
	    }                                   
	    ...
	    vector.pattern.21123[(((ind.R-1)*n.ite.B)+ind.B)] <-  length (which(data.RPL$patternStep3==21123))/n.choices
	    vector.pattern.23123[(((ind.R-1)*n.ite.B)+ind.B)] <-  length (which(data.RPL$patternStep3==23123))/n.choices
...
PROB.vector.pattern.21123  <- vector.pattern.21123/ n.individuals
PROB.vector.pattern.23123  <- vector.pattern.23123/ n.individuals
...
#Pattern (3,3,3,3,3)
if (quantile(PROB.vector.pattern.33333,0.025)<=OBSERV.vector.pattern.33333 & quantile(PROB.vector.pattern.33333,alpha1)>=OBSERV.vector.pattern.33333){
...
#Pattern (2,1,1,2,3)
if (quantile(PROB.vector.pattern.21123,0)<=OBSERV.vector.pattern.21123 & quantile(PROB.vector.pattern.21123,alpha1)>=OBSERV.vector.pattern.21123){
  TEST.PROB.pattern21123 <- "YES"
}else{
  TEST.PROB.pattern21123 <- "NO"
}

#Pattern (2,3,1,2,3)
if (quantile(PROB.vector.pattern.23123,0)<=OBSERV.vector.pattern.23123 & quantile(PROB.vector.pattern.23123,alpha1)>=OBSERV.vector.pattern.23123){
  TEST.PROB.pattern23123 <- "YES"
}else{
  TEST.PROB.pattern23123 <- "NO"
}


# Saves the results of the test for each of the patter
RESULTS <- matrix(NA,6,3 )
rownames(RESULTS) <- c(
  "pattern.11111",
  "pattern.22222",
  "pattern.33333",
...
colnames(RESULTS) <- c("observed frequency","95% quantile of the frequency", "fail to reject Ho?")
...
RESULTS["pattern.21123",] <- c(OBSERV.vector.pattern.21123,quantile(PROB.vector.pattern.21123,0.95),TEST.PROB.pattern21123)
RESULTS["pattern.23123",] <- c(OBSERV.vector.pattern.23123,quantile(PROB.vector.pattern.23123,0.95),TEST.PROB.pattern23123)
...
resultados_prob <- data.frame(PROB.vector.pattern.11111,PROB.vector.pattern.22222,PROB.vector.pattern.33333,PROB.vector.pattern.44444,PROB.vector.pattern.21123,PROB.vector.pattern.23123, weight)
...
# Pattern (2,1,1,2,3)
p5 <- ggplot(resultados_prob, aes(PROB.vector.pattern.21123, weight = weight)) + geom_histogram(color="black", fill="white", breaks=breaks1)+  
  labs(x="Pattern 21123", y="Frequency") + xlim(c(0,0.25)) + ylim(c(0,1))+
  geom_vline(xintercept = OBSERV.vector.pattern.21123, linetype="dotted", color = "black", size=1.5)+
  geom_vline(xintercept = quantile(PROB.vector.pattern.21123,0.95), linetype="longdash",color = "black", size=1.5)+ 
  theme_classic()+
  theme(text = element_text(size=25),
        plot.title = element_blank(),
        axis.title.x = element_blank(),
...
# Pattern (2,3,1,2,3)
p6 <- ggplot(resultados_prob, aes(PROB.vector.pattern.23123, weight = weight)) + geom_histogram(color="black", fill="white", breaks=breaks1)+  
  labs(x="Pattern 23123", y="Frequency") + xlim(c(0,0.25)) + ylim(c(0,1))+
  geom_vline(xintercept = OBSERV.vector.pattern.23123, linetype="dotted", color = "black", size=1.5)+
  geom_vline(xintercept = quantile(PROB.vector.pattern.23123,0.95), linetype="longdash",color = "black", size=1.5)+ 
  theme_classic()+
  theme(text = element_text(size=25),
        plot.title = element_blank(),
        axis.title.x = element_blank(),
